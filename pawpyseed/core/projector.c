...
ppot_t *get_projector_list(int num_els, int *labels, int *ls,
                           double *wave_grids, double *projectors,
                           double *aewaves, double *pswaves, double *rmaxs,
                           double grid_encut) {
  ...
  for (int i = 0; i < num_els; i++) {
    ...
    pps[i].total_projs = 0;
    pps[i].wave_grid =
        (double *)malloc((pps[i].wave_gridsize) * sizeof(double));
    pps[i].kwave_grid =
        (double *)malloc((pps[i].wave_gridsize) * sizeof(double));
    ...
    funcset_t *funcs =
        (funcset_t *)malloc(pps[i].num_projs * sizeof(funcset_t));
    CHECK_ALLOCATION(funcs);
    double *dense_wavegrid = (double *)malloc(
        DENSE_GRID_SCALE * pps[i].wave_gridsize * sizeof(double));
    CHECK_ALLOCATION(dense_wavegrid);
    dense_wavegrid[0] = pps[i].wave_grid[0];
    double factor =
        pow(pps[i].wave_grid[1] / pps[i].wave_grid[0], 1.0 / DENSE_GRID_SCALE);
    ...
    pps[i].smooth_grid =
        (double *)malloc(pps[i].proj_gridsize * sizeof(double));
    for (int j = 0; j < pps[i].proj_gridsize; j++) {
      pps[i].smooth_grid[j] = pps[i].wave_rmax / pps[i].proj_gridsize * j;
    }
    double *dense_kwavegrid = (double *)malloc(
        DENSE_GRID_SCALE * pps[i].wave_gridsize * sizeof(double));
    ...
    for (int k = 0; k < pps[i].num_projs; k++) {
      ...
      funcs[k].diffwave =
          (double *)malloc(sizeof(double) * pps[i].wave_gridsize);
      ...
      funcs[k].proj_spline =
          spline_coeff(pps[i].proj_grid, funcs[k].proj, pps[i].proj_gridsize);
      funcs[k].aewave_spline =
          spline_coeff(pps[i].wave_grid, funcs[k].aewave, pps[i].wave_gridsize);
      funcs[k].pswave_spline =
          spline_coeff(pps[i].wave_grid, funcs[k].pswave, pps[i].wave_gridsize);
      funcs[k].diffwave_spline = spline_coeff(
          pps[i].wave_grid, funcs[k].diffwave, pps[i].wave_gridsize);
    }

    sbt_descriptor_t *d = spherical_bessel_transform_setup(
        1e7, 0, pps[i].lmax, pps[i].wave_gridsize, pps[i].wave_grid,
        pps[i].kwave_grid);
    for (int k = 0; k < pps[i].num_projs; k++) {
      funcs[k].kwave =
          wave_spherical_bessel_transform(d, funcs[k].diffwave, funcs[k].l);
      // funcs[k].kwave = besselt(pps[i].wave_grid, pps[i].kwave_grid,
      // funcs[k].diffwave, 520.0, pps[i].wave_gridsize, funcs[k].l);
      funcs[k].kwave_spline =
          spline_coeff(pps[i].kwave_grid, funcs[k].kwave, pps[i].wave_gridsize);
    }
    for (int k = 0; k < pps[i].num_projs; k++) {
      // double* dense_kwave = wave_spherical_bessel_transform(d2,
      // funcs[k].smooth_diffwave, funcs[k].l);
      double *dense_kwave = (double *)calloc(
          pps[i].wave_gridsize * DENSE_GRID_SCALE, sizeof(double));
      int q = 0;
      ...
      double *smooth_diffwave =
          inverse_wave_spherical_bessel_transform(d, dense_kwave, funcs[k].l);
      double **smooth_wave_spline =
      ...
      for (int p = 1; p < pps[i].proj_gridsize; p++) {
        // smooth_grid should be like proj_grid except that rmax should be rmax
        // of the partial wave difference
        sdw[p] = wave_interpolate(
            pps[i].smooth_grid[p], pps[i].wave_gridsize * DENSE_GRID_SCALE,
            dense_wavegrid, smooth_diffwave, smooth_wave_spline);
      }
      if (funcs[k].l == 0)
        sdw[0] = sdw[1];
      double **sdw_spline =
          spline_coeff(pps[i].smooth_grid, sdw, pps[i].proj_gridsize);
...
real_proj_site_t *projector_values(int num_sites, int *labels, double *coords,
  ...
  real_proj_site_t *sites =
      (real_proj_site_t *)malloc(num_sites * sizeof(real_proj_site_t));
  CHECK_ALLOCATION(sites);
  int *all_sites = (int *)malloc(num_sites * sizeof(int));
  for (int i = 0; i < num_sites; i++) {
    all_sites[i] = i;
  }
  setup_site(sites, pps, num_sites, all_sites, labels, coords, lattice, fftg,
             0);
...
real_proj_site_t *smooth_pw_values(int num_N, int *Nlst, int *labels,
                                   double *coords, double *lattice,
                                   double *reclattice, ppot_t *pps, int *fftg) {

  real_proj_site_t *sites =
      (real_proj_site_t *)malloc(num_N * sizeof(real_proj_site_t));
  CHECK_ALLOCATION(sites);
...
void onto_projector_helper(band_t *band, double complex *x,
                           real_proj_site_t *sites, int num_sites,
                           double *lattice, double *reclattice, double *kpt,
                           int num_cart_gridpts, int *fftg,
  ...
  double complex *xvals =
      (double complex *)malloc(num_cart_gridpts * sizeof(double complex));
  int *indices;
  ...
  for (int s = 0; s < num_sites; s++) {
    ...
    projections[s].overlaps =
        (double complex *)malloc(sites[s].total_projs * sizeof(double complex));
...
void get_aug_freqs_helper(band_t *band, double complex *x,
                          real_proj_site_t *sites, int num_sites,
                          double *lattice, double *reclattice, double *kpt,
                          int num_cart_gridpts, int *fftg,
                          projection_t *projections) {
  ...
  for (int s = 0; s < num_sites; s++) {
    ...
    for (int ind = 0; ind < num_indices; ind++) {
      ...
      for (int p = 0; p < sites[s].total_projs; p++) {
        values = sites[s].projs[p].values;
        x[index] +=
            projections[sites[s].index].overlaps[p] * values[ind] * phase;
      }
    }
  }
}

void onto_projector(kpoint_t *kpt, int band_num, real_proj_site_t *sites,
                    int num_sites, int *G_bounds, double *lattice,
                    double *reclattice, int num_cart_gridpts, int *fftg) {
  ...
  onto_projector_helper(kpt->bands[band_num], x, sites, num_sites, lattice,
...
void onto_projector_ncl(kpoint_t *kpt, int band_num, real_proj_site_t *sites,
                        int num_sites, int *G_bounds, double *lattice,
                        double *reclattice, int num_cart_gridpts, int *fftg) {
  ...
  double complex *xup = (double complex *)mkl_calloc(
      fftg[0] * fftg[1] * fftg[2], sizeof(double complex), 64);
  double complex *xdown = (double complex *)mkl_calloc(
      fftg[0] * fftg[1] * fftg[2], sizeof(double complex), 64);
  CHECK_ALLOCATION(xup);
  CHECK_ALLOCATION(xdown);
  fft3d(xup, G_bounds, lattice, k, Gs, Cs, num_waves / 2, fftg);
  fft3d(xdown, G_bounds, lattice, k, Gs, Cs + num_waves / 2, num_waves / 2,
        fftg);
  ...
  band->down_projections =
      (projection_t *)malloc(num_sites * sizeof(projection_t));
  onto_projector_helper(kpt->bands[band_num], xup, sites, num_sites, lattice,
                        reclattice, k, num_cart_gridpts, fftg,
                        band->up_projections);
...
void onto_smoothpw(kpoint_t *kpt, int band_num, real_proj_site_t *sites,
                   int num_sites, int *G_bounds, double *lattice,
                   double *reclattice, int num_cart_gridpts, int *fftg) {
...
void get_aug_freqs(kpoint_t *kpt, int band_num, real_proj_site_t *sites,
                   int num_sites, int *G_bounds, double *lattice,
                   double *reclattice, int num_cart_gridpts, int *fftg) {
  ...
  get_aug_freqs_helper(kpt->bands[band_num], x, sites, num_sites, lattice,
                       reclattice, k, num_cart_gridpts, fftg,
                       band->projections);

  band->CAs =
      (float complex *)mkl_calloc(kpt->num_waves, sizeof(float complex), 64);
  fwd_fft3d(x, G_bounds, lattice, k, Gs, band->CAs, num_waves, fftg);
  // for (int w = 0; w < kpt->num_waves; w++) {
  //	band->CAs[w] += band->Cs[w];
  //		printf("%f %f %f %f\n", creal(band->CAs[w]),
  // cimag(band->CAs[w]), 		creal(band->Cs[w]), cimag(band->Cs[w]));
  //}
  ...
  for (int i = 0; i < pp.num_projs; i++) {
    for (int j = i; j < pp.num_projs; j++) {
      if (pp.funcs[i].l == pp.funcs[j].l) {
        ...
        double **psspline =
            spline_coeff(pp.wave_grid, psprod, pp.wave_gridsize);
        double **aespline =
            spline_coeff(pp.wave_grid, aeprod, pp.wave_gridsize);
        double **displine =
            spline_coeff(pp.wave_grid, diprod, pp.wave_gridsize);
        psov[i * pp.num_projs + j] =
            spline_integral(pp.wave_grid, psprod, psspline, pp.wave_gridsize);
        aeov[i * pp.num_projs + j] =
            spline_integral(pp.wave_grid, aeprod, aespline, pp.wave_gridsize);
        diov[i * pp.num_projs + j] =
            spline_integral(pp.wave_grid, diprod, displine, pp.wave_gridsize);
      }
    }
  }
...
void setup_projections(pswf_t *wf, ppot_t *pps, int num_elems, int num_sites,
  ...
  real_proj_site_t *sites = projector_values(
      num_sites, labels, coords, wf->lattice, wf->reclattice, pps, fftg);
...
#pragma omp parallel for
  for (int w = 0; w < NUM_BANDS * NUM_KPTS; w++) {
    ...
    onto_projector(kpt, band_num, sites, num_sites, wf->G_bounds, wf->lattice,
                   wf->reclattice, num_cart_gridpts, fftg);
    if (wf->is_ncl) {
      onto_projector_ncl(kpt, band_num, sites, num_sites, wf->G_bounds,
                         wf->lattice, wf->reclattice, num_cart_gridpts, fftg);
    }
  }
  printf("Done \n");
...
void overlap_setup_real(pswf_t *wf_R, pswf_t *wf_S, int *labels_R,
                        int *labels_S, double *coords_R, double *coords_S,
                        int *N_R, int *N_S, int *N_RS_R, int *N_RS_S,
                        int num_N_R, int num_N_S, int num_N_RS) {
  ...
  if (num_N_R > 0) {
    real_proj_site_t *sites_N_R =
        smooth_pw_values(num_N_R, N_R, labels_R, coords_R, wf_S->lattice,
                         wf_S->reclattice, wf_R->pps, wf_S->fftg);
...
#pragma omp parallel for
    for (int w = 0; w < NUM_BANDS * NUM_KPTS; w++) {
      ...
      onto_smoothpw(kpt_S, w / NUM_KPTS, sites_N_R, num_N_R, wf_S->G_bounds,
                    wf_S->lattice, wf_S->reclattice, max_num_indices,
                    wf_S->fftg);
    }
  ...
  printf("PART 1 DONE\n");
  if (num_N_S > 0) {
    real_proj_site_t *sites_N_S =
        smooth_pw_values(num_N_S, N_S, labels_S, coords_S, wf_R->lattice,
                         wf_R->reclattice, wf_S->pps, wf_R->fftg);
...
#pragma omp parallel for
    for (int w = 0; w < NUM_BANDS * NUM_KPTS; w++) {
      ...
      onto_smoothpw(kpt_R, w / NUM_KPTS, sites_N_S, num_N_S, wf_R->G_bounds,
                    wf_R->lattice, wf_R->reclattice, max_num_indices,
                    wf_R->fftg);
    }
...
#pragma omp parallel for
  for (int i = 0; i < num_N_RS; i++) {
    ...
    // CALCULATE THE DIFF COORD HERE, PASS TO offsite_wave_overlap AND SAVE IT
    // FOR USE IN compensation_terms
    overlaps[i] = (double complex *)calloc(pp1.total_projs * pp2.total_projs,
        ...
        for (int k = 0; k < pp2.num_projs; k++) {
          ...
          for (int m2 = -l2; m2 <= l2; m2++) {
            overlaps[i][tj * pp2.total_projs + tk] =
                conj(reciprocal_offsite_wave_overlap(
                    dcoords + 3 * i, pp1.kwave_grid, pp1.funcs[j].kwave,
                    pp1.funcs[j].kwave_spline, pp1.wave_gridsize,
                    pp2.kwave_grid, pp2.funcs[k].kwave,
                    pp2.funcs[k].kwave_spline, pp2.wave_gridsize, wf_R->lattice,
                    l1, m1, l2, m2));
            tk++;
          }
        }
...
void overlap_setup_recip(pswf_t *wf_R, pswf_t *wf_S, int *labels_R,
                         int *labels_S, double *coords_R, double *coords_S,
                         int *N_R, int *N_S, int *N_RS_R, int *N_RS_S,
                         int num_N_R, int num_N_S, int num_N_RS) {
  ...
  if (num_N_R > 0) {
    real_proj_site_t *sites_N_R =
        smooth_pw_values(num_N_R, N_R, labels_R, coords_R, wf_R->lattice,
                         wf_R->reclattice, wf_R->pps, wf_R->fftg);
    for (int s = 0; s < num_N_R; s++) {
      if (sites_N_R[s].num_indices > max_num_indices) {
        max_num_indices = sites_N_R[s].num_indices;
      }
...
#pragma omp parallel for
    for (int w = 0; w < NUM_BANDS * NUM_KPTS; w++) {
      ...
      get_aug_freqs(kpt_R, w / NUM_KPTS, sites_N_R, num_N_R, wf_R->G_bounds,
                    wf_R->lattice, wf_R->reclattice, max_num_indices,
                    wf_R->fftg);
    }
    free_real_proj_site_list(sites_N_R, num_N_R);
  ...
  if (num_N_S > 0) {
    real_proj_site_t *sites_N_S =
        smooth_pw_values(num_N_S, N_S, labels_S, coords_S, wf_S->lattice,
                         wf_S->reclattice, wf_S->pps, wf_S->fftg);
...
#pragma omp parallel for
    for (int w = 0; w < NUM_BANDS * NUM_KPTS; w++) {
      ...
      get_aug_freqs(kpt_S, w / NUM_KPTS, sites_N_S, num_N_S, wf_S->G_bounds,
                    wf_S->lattice, wf_S->reclattice, max_num_indices,
                    wf_S->fftg);
    }
    free_real_proj_site_list(sites_N_S, num_N_S);
...
void compensation_terms(double complex *overlap, int BAND_NUM, pswf_t *wf_S,
                        pswf_t *wf_R, int num_M, int num_N_R, int num_N_S,
                        int num_N_RS, int *M_R, int *M_S, int *N_R, int *N_S,
                        int *N_RS_R, int *N_RS_S, int *proj_labels,
                        double *proj_coords, int *ref_labels,
                        double *ref_coords, int *fft_grid, int flip_spin) {
  ...
  // double* overlap = (double*) calloc(2 * NUM_KPTS * NUM_BANDS,
  // sizeof(double));
...
#pragma omp parallel for
  for (int w = 0; w < NUM_BANDS * NUM_KPTS; w++) {
    ...
    // overlap[2*w] += creal(temp);
    // overlap[2*w+1]+= cimag(temp);
    // printf("temp 3 %d %d %d %lf %lf\n", kpt_S->num_waves, kpt_R->num_waves,
    // w%NUM_KPTS, creal(temp), cimag(temp));
      ...
      for (int i = 0; i < pron.total_projs; i++) {
        for (int j = 0; j < ppron.total_projs; j++) {
          temp += conj(pron.overlaps[i]) *
                  ...
                  ppron.overlaps[j] *
                  cexp(2 * I * PI * dot(kpt_R->k, wf_S->dcoords + 3 * s));
        }
      }
    }
    overlap[w] += temp;
...
void compensation_terms_recip(double complex *overlap, int BAND_NUM,
                              pswf_t *wf_S, pswf_t *wf_R, int num_M,
                              int num_N_R, int num_N_S, int num_N_RS, int *M_R,
                              int *M_S, int *N_R, int *N_S, int *N_RS_R,
                              int *N_RS_S, int *proj_labels,
                              double *proj_coords, int *ref_labels,
                              double *ref_coords, int *fft_grid,
                              int flip_spin) {
...
#pragma omp parallel for
  for (int w = 0; w < NUM_BANDS * NUM_KPTS; w++) {
    ...
    // w%NUM_KPTS, 	creal(overlap[w]), cimag(overlap[w]),
    // creal(curr_overlap),
    // cimag(curr_overlap));
