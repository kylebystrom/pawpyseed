...
pswf_t* expand_symm_wf(pswf_t* rwf, int num_kpts, double* kpts, int* maps, double* ops) {
	...
	for (int knum = 0; knum < num_kpts * wf->nspin; knum++) {
		...
		kpt->k = (double*) malloc(3 * sizeof(double));
		transform_vector(kpt->k, ops[knum], rkpt->k);
		kpt->Gs = (int*) malloc(3 * kpt->num_waves * sizeof(int));

		kpt->wight = rkpt->weight;
		...
		for (int g = 0; g < kpt->num_waves; g++) {
			...
			transform_vector(pw, ops[knum], pw);
