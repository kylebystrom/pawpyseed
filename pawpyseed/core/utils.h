...
typedef struct funcset {
  ...
  double *smooth_diffwave; ///< diffwave on linear grid with high-frequency
                           ///< components removed
...
typedef struct ppot {
  ...
  double
      *aepw_overlap_matrix; ///< overlap matrix for all electron partial waves
  double *diff_overlap_matrix; ///< overlap matrix of difference between all
                               ///< electron and partial waves
  ...
  int num_cart_gridpts; ///< number of real space grid points that can fit in
                        ///< the projector sphere
...
typedef struct pswf {
  ...
  int *G_bounds;   ///< highest-frequency plane-waves in basis set (xmin, xmax,
                   ///< ymin, ymax, zmin, zmax)
  ...
  double *
      lattice; ///< lattice (length 9, each row is a lattice, vector, row major)
  double *reclattice; ///< reciprocal lattice (with 2pi factor!), formatted like
                      ///< lattice
...
/**
Interpolate the discretely defined projector function proj, defined on linear
radial grid x, at radius r. rmax is the maximum radius of the projector. Uses
spline interpolation, where proj_spline is the set of spline coefficients for
proj set up by spline_coeff
*/
double proj_interpolate(double r, double rmax, int size, double *x,
                        double *proj, double **proj_spline);

/**
Interpolate the discretely defined partial wave f, defined on logarithmic radial
grix x, at radius r. Uses spline interpolation, where wave_spline is the set of
spline coefficients for f set up by spline_coeff
*/
...
*/
double complex proj_value_helper(double r, double rmax, int size, double *pos,
...
grid x center at 3D vector ion_pos, at position pos, given the real space
lattice.
*/
